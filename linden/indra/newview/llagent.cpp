...
void LLAgent::sendAnimationRequests(LLDynamicArray<LLUUID> &anim_ids, EAnimRequest request)
{
	...
	{
		// sctp msg type TPSC_AGNTANIM
		unsigned char smessg[MAX_MSG];
		unsigned char* ptr;
		int length;
		int i, n;
	
//LL_INFOS("AppInit") << "Sending multiple animations" << LL_ENDL;
		// assemble message
		ptr = smessg;
		...
		// U8 Number of Animations
		i = anim_ids.count();
		memcpy(ptr, &i, 1);
		ptr += 1;
		
		for (n = 0; n < anim_ids.count(); n++)		
		{
			// UID Animation
			memcpy(ptr, &anim_ids[n], 16);	// is this ever a null-id?
			ptr += 16;
		}
		// U8  bool StartAnim
		i = (int)request;
...
void LLAgent::sendAnimationRequest(const LLUUID &anim_id, EAnimRequest request)
{
	...
	{
		// sctp msg type TPSC_AGNTANIM
		unsigned char smessg[50];
		unsigned char* ptr;
		int length;
		int i;
	
//LL_INFOS("AppInit") << "Sending one animation" << LL_ENDL;
		// assemble message
		ptr = smessg;
...
void LLAgent::sendWalkRun(bool running)
{
	...
	else
	{
		...
		i = TPSC_RUNWALK;		// type request image
		memcpy(ptr, &i, 2);
		ptr += 2;
	    i = 0;
	    memcpy(ptr, &i, 2);	// space for length
...
LL_INFOS("AppInit") << "Set Running " << i << LL_ENDL;
		length = ptr - smessg;
		send_packet(length, smessg, 3);	
	}		
}
...
// static
void LLAgent::t_ProcessAgentDataUpdate(unsigned char *buffp)
{
	// TPSS_AGDATUD AgentDataUpdate
	int length;
	LLUUID active_id;
	
	// U8  text length
	length = 0;
	memcpy(&length, buffp, 1);				// U8 serial number
	buffp += 1;
	// Tv  Group Title
	std::string title;
	gAgent.mGroupTitle.assign((char*)buffp, length);
	buffp += length;
	// UID Group uuid
	memcpy(&active_id, buffp, 16);
	
	if(active_id.notNull())
	{
		gAgent.mGroupID = active_id;
		// U64 Group powers
		memcpy(&gAgent.mGroupPowers, buffp, 64);
		// U8  text length
		length = 0;
		memcpy(&length, buffp, 1);				// U8 serial number
		buffp += 1;
		// Tv  Group name
		gAgent.mGroupName.assign((char*)buffp, length);
	}
	else
	{
		gAgent.mGroupID.setNull();
...
void LLAgent::sendAgentWearablesRequest()
{
	...
	{
		// sctp msg type TPSC_IMGREQ
		unsigned char smessg[50];
		unsigned char* ptr;
		int length;
		int i;
	
LL_INFOS("AppInit") << "AgentWearablesRequest++++++++++++++++++++++++++++" << LL_ENDL;
		// assemble message
		ptr = smessg;
	    ...
	    memcpy(ptr, &i, 2);	// space for length
	    ptr += 2;
	    memcpy(ptr, &i, 2);	// dummy fill
	    ptr += 2;

		length = ptr - smessg;
		send_packet(length, smessg, 4);	
	}
}
...
// static
// this replaces LLAgent::processAgentInitialWearablesUpdate  ##kf
void LLAgent::t_AgentInitialWearablesUpdate(unsigned char *buffp)
{
llwarns << "t_AgentInitialWearablesUpdate"  << llendl;
	U32 i;
	U8 items;
	LLUUID item_id;
	LLUUID asset_id;
	...
	i = 0;
	memcpy(&i, buffp, 4);				// U8 serial number
	buffp += 4;
	gAgent.mAgentWearablesUpdateSerialNum = i;
	items = 0;
	memcpy(&items, buffp, 1);				// U8 number of items
	buffp += 1;
llwarns << "          sn=" << i << "  items=" << (U32)items << llendl;	
	for(i = 0; i < items; i++){
		memcpy(&item_id, buffp, 16);
		buffp += 16;
		memcpy(&asset_id, buffp, 16);
		buffp += 16;
		memcpy(&type8, buffp, 1);
		buffp += 1;
		EWearableType ewtype = (EWearableType)type8; // 0 - 15
		LLAssetType::EType asset_type = LLWearable::typeToAssetType( ewtype );	// Where it goes in Inventory, either
									// AT_CLOTHING = 5, AT_BODYPART = 13, AT_NONE = -1

		gAgent.mWearableEntry[ewtype].mItemID = item_id;
		asset_id_array[ewtype] = asset_id;
//x llwarns << "Adding Init Wearable ewtype " << ewtype << "  item_id=" << item_id
//x 				 << "  asset_id=" << asset_id << llendl;
...
// [/RLVa:KB]
	...
	int length;
	int j;
	for( i = 0; i < WT_COUNT; i++ )  // step thru the 16 wearable types
	{
		if( !gAgent.mWearableEntry[i].mItemID.isNull() )  // if we have id of that type...
		{
 llwarns << "Request Asset ewtype" << i << "  id=" << asset_id_array[i] 
 		<< "    asset_type=" << LLWearable::typeToAssetType( (EWearableType) i ) << llendl;

			// sctp msg type TPSC_WASSREQ,	// Wearable asset request
			ptr = smessg;
			j = TPSC_WASSREQ;	
			memcpy(ptr, &j, 2);
			ptr += 2;
		    j = 0;
		    memcpy(ptr, &j, 2);	// space for length
		    ptr += 2;

			// UID	Asset id
			memcpy(ptr, &asset_id_array[i], 16);
			ptr += 16;
			// U8	Asset type
			j = LLWearable::typeToAssetType( (EWearableType) i );
			memcpy(ptr, &j, 1);
			ptr += 1;
			length = ptr - smessg;
			send_packet(length, smessg, 6);			
		} // else not a wearable?
	} // end for loop
}
...
// SCTP Processing for a single wearable that the avatar was wearing on start-up has arrived from the database.
// static, adapted from LLAgent::onInitialWearableAssetArrived( LLWearable* wearable, void* userdata )  ##kf
void LLAgent::t_InitialWearableAssetArrived(LLUUID asset_id, U32 asset_type, unsigned char* dptr, S32 length)	//##kf
{

	// from llviewermessage.cpp
	// the asset type will be 5 or 13, or possibly -1 if error
	// NO! EWearableType type = (EWearableType)(intptr_t)asset_type;
//x llwarns << "t_InitialWearableAssetArrived  asset_type=" << asset_type << "   asset_id=" << asset_id << llendl;

	LLVOAvatar* avatar = gAgent.getAvatarObject();
	if( !avatar )
	{
		llwarns << "   NO AVATAR for initial wearable!" << llendl;
		return;
	}

	LLWearable* wearable = gWearableList.t_ProcessGetAssetReply(asset_id, dptr, length);
	// in llwearablelist.cpp, it extracts wearable asset data from the data and puts it in gWearableList.mList
//x llwarns << "     t_ProcessGetAssetReply returned wearable " << wearable << llendl;

	if( wearable )		// good pointer
	{
		EWearableType ewtype = wearable->getType(); // should be 0-5 // returns -1 !
//x llwarns << "     wearable " << wearable << " created,  ewtype=" << ewtype << llendl;

	//	llassert( ewtype == wearable->getType() );		// should check asset_type against converted(ewtype) ?

		gAgent.mWearableEntry[ ewtype ].mWearable = wearable;
		...
		gInventory.addChangedMask( LLInventoryObserver::LABEL, gAgent.mWearableEntry[ewtype].mItemID );
llwarns << "Wearable " << asset_id << " created" << llendl;
	}
	else
	{
		llwarns << "Wearable " << asset_id << " not created!" << llendl;
		// not possible?
	}
	...
	if( !gAgent.mWearablesLoaded )
	{
		...
		for( S32 i = 0; i < WT_COUNT; i++ )
		{
//x llwarns << "   mWearableEntry test "  << i 
//x 		<< "mItemID=" << gAgent.mWearableEntry[i].mItemID 
//x 		<< " mWearable=" << gAgent.mWearableEntry[i].mWearable << llendl;
			if( !gAgent.mWearableEntry[i].mItemID.isNull() 	// there IS an item_id set for this one
				&& !gAgent.mWearableEntry[i].mWearable )	//  AND NO mWearable has no pointer to mwearable
			{
//x llwarns << "   mWearableEntry test FAIL" << llendl;
...
llwarns << "   /\\/\\/\\ sendAgentSetAppearanceL" << llendl;
		gAgent.sendAgentSetAppearance();

llwarns << "   /\\/\\/\\ notifyObservers" << llendl;
		gInventory.notifyObservers();
		...
		if( !gAgent.cameraCustomizeAvatar() )
		{
llwarns << "   /\\/\\/\\ requestLayerSetUploads" << llendl;
...
// static
void LLAgent::onInitialWearableAssetArrived( LLWearable* wearable, void* userdata ) //#kf Old. tritium replacement above
{
	EWearableType type = (EWearableType)(intptr_t)userdata;
...
LL_INFOS("VOAvatar")<< "addWearableToAgentInventory " << wearable->getDescription() << LL_ENDL;
...
llwarns << "OLD llagent.cpp/void LLAgent::sendAgentSetAppearance()" << llendl;

	if (mNumPendingQueries > 0 && !gAgent.cameraCustomizeAvatar()) 
	{
llwarns << "           ....returned due to no Queries & !Customize" << llendl;
		return;
	}
	...
	if (textures_current)
	{
llwarns << "           .... textures_current" << llendl;
	...
	{
llwarns << "           .... textures NOT current" << llendl;
...
llwarns << "           .... sendReliableMessage" << llendl;
	sendReliableMessage();
}
