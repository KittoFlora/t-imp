...
//static 
void LLViewerImageList::SCTPreceiveImageHeader(LLUUID id, U8 codec, U16 packets, U32 totalbytes, S32 data_size, U8 *data)		// ##kft added
{
	LLFastTimer t(LLFastTimer::FTM_PROCESS_IMAGES);
	LLHost host;
	
//llwarns << "SCTPreceiveImageHeader for " << id  << llendl;
//llwarns << "  Codec:    " << codec   << llendl;
//llwarns << "  Pkts:     " << packets   << llendl;
//llwarns << "  TotBy:    " << totalbytes   << llendl;
//llwarns << "  data_size:" << data_size  << llendl;

	host.set("169.254.0.1", 5555);

	gImageList.sTextureBits += data_size * 8;
	gImageList.sTexturePackets++;
	...
	LLViewerImage *image = gImageList.getImage(id);
llwarns << "List Image:" << image << llendl;
	if (!image)
	{
		llerrs << "Error? No 'image' for " << id << llendl;
		return;
	}
	...
	image->mLastPacketTimer.reset();
//llwarns << "TimerReset " << llendl;
	bool res = LLAppViewer::getTextureFetch()->receiveImageHeader(host, id, codec, packets, totalbytes, data_size, data2);

	if (!res)
	{
		llwarns << "ERROR: receiveImageHeader failed! " << llendl;
		delete[] data2;
	}
}
...
// static
void LLViewerImageList::SCTPreceiveImagePacket(LLUUID id, U16 packet_num, S32 data_size, U8 *data)  //##kft
{
	LLMemType mt1(LLMemType::MTYPE_APPFMTIMAGE);
	LLFastTimer t(LLFastTimer::FTM_PROCESS_IMAGES);
	LLHost host;
	...
	U32 brec = gImageList.sTextureBits / 8;


if(data_size != 1000){
  LL_WARNS("AppInit") << "SCTPreceiveImagePacket " << packet_num << 
		"    Size:" << data_size  <<
		"    for " << id << llendl; //##kft
}
	...
	image->mLastPacketTimer.reset();
	bool res = LLAppViewer::getTextureFetch()->receiveImagePacket(host, id, packet_num, data_size, data2);
	if (!res)
	{
		llwarns << "ERROR: receiveImagePacket failed! " << llendl;
		delete[] data2;
	}
}
...
void LLViewerImageList::SCTPImageNotInDatabase(LLUUID image_id)
{
	LLFastTimer t(LLFastTimer::FTM_PROCESS_IMAGES);
