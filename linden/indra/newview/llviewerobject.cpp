...
//LL_INFOS("AppInit") << "In createObject, pcode=" << pcode << LL_ENDL;		
//LL_INFOS("AppInit") << "   id=" << id << "  regionp=" <<  regionp << LL_ENDL;
...
//LL_INFOS("AppInit") << "    LL_PCODE_LEGACY_AVATAR" << LL_ENDL;
	  res = new LLVOAvatar(id, pcode, regionp); 
	  break;
	case LL_PCODE_LEGACY_GRASS:
//LL_INFOS("AppInit") << "    LL_PCODE_LEGACY_GRASS" << LL_ENDL;
...
//##kf This is the tritium handler for Avatar terse update, adapted from old processUpdateMessage()
U32 LLViewerObject::t_ProcessUpdateMessage(U64 region_handle,			//##kf
									   	   U16 time_dilation16,
										   ...
										   U32 obj_num)
{
	...
	LLMemType mt(LLMemType::MTYPE_OBJECT);	// ?? not used
	U32 retval = 0x0;
	
	// Coordinates of objects on simulators are region-local.
// import U64 region_handle;
...
//	S32	count;
	...
	// only handling OUT_TERSE_IMPROVED:
	...
	// U8 Object state = 0
	U8 state = 0;
	memcpy(&state, bufferp, 1);
	bufferp += 1;
	mState = state;

	// U8 Is an agent? = 1
	U8 agent = 0;
	memcpy(&agent, bufferp, 1);
	bufferp += 1;

	// contents of data block:
	// (4xF32) Collision plane
	// (3xF32) Position
	// (3xU16) Quantized velocity (-128.f to 128.f)
	// (3xU16) Quantized acceleration (-64.f to 64.f)
	// (4xU16) Quantized quaternion rotation (-1.f to 1.f)
	// (3xU16) Quantized rotational velocity (-64.f to 64.f) 
	// 	U16 Textures block count = 0 for an agent		
				
	// pull out collision normal for avatar
	// (4xF32) Collision plane
	memcpy(&collision_plane.mV[0], bufferp, 4);
	bufferp += 4;
	memcpy(&collision_plane.mV[1], bufferp, 4);
	bufferp += 4;
	memcpy(&collision_plane.mV[2], bufferp, 4);
	bufferp += 4;
	memcpy(&collision_plane.mV[3], bufferp, 4);
	bufferp += 4;
	
	U32 s = (U32)state;
	U32 agt = (U32)agent;
//llwarns << "   state=" << s << "  isAgt=" << agt << "  CollPln=" << collision_plane.mV[0] <<
//		"," << collision_plane.mV[1] << "," << collision_plane.mV[2] << "," << collision_plane.mV[3] << llendl;

	((LLVOAvatar*)this)->setFootPlane(collision_plane);

	this_update_precision = 32;
	// (3xF32) Position
	memcpy(&new_pos_parent.mV[VX], bufferp, 4);
	bufferp += 4;
	memcpy(&new_pos_parent.mV[VY], bufferp, 4);
	bufferp += 4;
	memcpy(&new_pos_parent.mV[VZ], bufferp, 4);
	bufferp += 4;

//llwarns << "   pos=" << new_pos_parent.mV[VX] << ","<< new_pos_parent.mV[VY] << ","<< new_pos_parent.mV[VZ] << llendl;
...
//	test_pos_parent.quantize16(-0.5f*size, 1.5f*size, MIN_HEIGHT, MAX_HEIGHT);
	...
	// vel
	// (3xU16) Quantized velocity (-128.f to 128.f)
	memcpy(&valx, bufferp, 2);
	bufferp += 2;
	memcpy(&valy, bufferp, 2);
	bufferp += 2;
	memcpy(&valz, bufferp, 2);
	bufferp += 2;
	setVelocity(U16_to_F32(valx, -size, size),
				U16_to_F32(valy, -size, size),
				U16_to_F32(valz, -size, size));

//llwarns << "   Vel=" << valx << "," << valy << "," << valz << llendl;
	...
	memcpy(&valz, bufferp, 2);
	bufferp += 2;
	setAcceleration(U16_to_F32(valx, -size, size),
					U16_to_F32(valy, -size, size),
					U16_to_F32(valz, -size, size));

//llwarns << "   Acc=" << valx << "," << valy << "," << valz << llendl;
	// theta
	// (4xU16) Quantized quaternion rotation (-1.f to 1.f)
	memcpy(&valx, bufferp, 2);
	bufferp += 2;
	...
	memcpy(&valz, bufferp, 2);
	bufferp += 2;
	memcpy(&valw, bufferp, 2);
	bufferp += 2;
	new_rot.mQ[VX] = U16_to_F32(valx, -1.f, 1.f);
	new_rot.mQ[VY] = U16_to_F32(valy, -1.f, 1.f);
	new_rot.mQ[VZ] = U16_to_F32(valz, -1.f, 1.f);
	new_rot.mQ[VW] = U16_to_F32(valw, -1.f, 1.f);
//llwarns << "   Rot=" << valx << "," << valy << "," << valz << "," << valw << llendl;
	...
	memcpy(&valz, bufferp, 2);
	bufferp += 2;
	setAngularVelocity(	U16_to_F32(valx, -size, size),
						U16_to_F32(valy, -size, size),
						U16_to_F32(valz, -size, size));

//llwarns << "   RoV=" << valx << "," << valy << "," << valz << llendl;


	// U16 Textures block count = 0 for an agent
	U16 textures = 0;
	memcpy(&textures, bufferp, 2);
	bufferp += 2;

	// block is always 1 so we only ever call the above unpack once

//llwarns << "   textures=" << textures << llendl;
	...
	if(agent != 1)
	{
		llwarns << "Terse Update Unpack for non-agent!" << llendl;
	}

	if(textures != 0)
	{
		llwarns << "Terse Update Unpack sync error? Non-zero textures! = " << textures << llendl;
	}
	...
	mLatestRecvPacketID = 1; // can we ignore this?

	// Set the change flags for scale - no, ther's no scale change in this message
