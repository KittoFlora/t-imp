	...
	F64 timeout)
{
	...
	if (gSCTP > 0)		//##kf send by sctp / tcp
	{
		...
		int blocknumber;
		int remainder;
		int MAX_BLOCK = 4000;	// 4096 is the abs max?

		LLVFile vfile(mVFS, asset_id, asset_type, LLVFile::READ);
		S32 asset_size = vfile.getSize();

		blocks = asset_size / MAX_BLOCK;
		remainder = asset_size - (blocks * MAX_BLOCK);
		if(remainder > 0) blocks++;
		blocknumber = 0;

LL_WARNS("AppInit ") << "LViewerAssetStorage::storeAssetData SCTP "  << LLAssetType::lookup(asset_type)
			<< " Baked Image type " << asset_type			
			<< " Length=" << asset_size
			<< " Blocks=" << blocks
			<< " ASSET_ID:" << asset_id 
			<< LL_ENDL;
		...
		// UID	Asset id
		memcpy(ptr, &asset_id, 16);
		ptr += 16;			//22
		// U8	Asset type
		memcpy(ptr, &asset_type, 1);
		ptr += 1;			//23
		...
		// Tv	data bytes
		BOOL res = vfile.read(ptr, i);	
		ptr += i;
		
		length = ptr - smessg; //34 - 2033
		send_packet(length, smessg, 23);
//LL_WARNS("AppInit ") << "    Block 1" << LL_ENDL;
		blocknumber = 1; // so blocknumber == Total blocks when done
		blocks--;

		while (blocks > 0){
			ms_sleep(20);		// temp fix for choking //KF# This should run on a handshake from server
			ptr = smessg;
			i = TPSC_BIMBLK;	
			memcpy(ptr, &i, 2);
			ptr += 2;
			...
			length = ptr - smessg;
			send_packet(length, smessg, 24);

			blocknumber++;
			blocks--;
		}
LL_WARNS("AppInit ") << "   Last Block " << blocknumber << LL_ENDL;
			
	  //##kf end of sctp method
	}
	else
	{
...
//LL_WARNS("AppInit ") << "Size = " << asset_size 
//		<< ", Max=" << (MTUBYTES - 100)
//		<< "  Tmp=" << temp_file
//		<< "  Pri=" << is_priority
//		<< "  Loc=" << store_local
	...
	F64 timeout)
{
	...
	LLAssetID asset_id = tid.makeAssetID(gAgent.getSecureSessionID());
	llinfos << "LLViewerAssetStorage::storeAssetData (legacy)" << asset_id << ":" 
		<< LLAssetType::lookup(asset_type) << "  Name " << filename << llendl;
...
llwarns << "Call LLViewerAssetStorage::storeAssetData A" << llendl;
