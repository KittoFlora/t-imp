...
// 			req->setPriority(LLWorkerThread::PRIORITY_HIGH | req->mWorkPriority);
		 ...
		 iter1 != requests.end(); ++iter1)
	{
		...
		// step thru requests for this host
			 ...
			 iter2 != iter1->second.end(); ++iter2)
		{
			...
			if (gSCTP > 0) 
			{
				...
				if (req->mSentRequest == LLTextureFetchWorker::SENT_SIM) break;
				
				// sctp stream 5, type TPSC_IMGREQ
				unsigned char smessg[50];
				unsigned char* ptr;
				int length;
				int i;
	
LL_INFOS("AppInit") << "REQUESTING IMAGE" << req->mID << LL_ENDL;
				// assemble message
				ptr = smessg;
					
				i = TPSC_IMGREQ;		// type request image
				memcpy(ptr, &i, 2);
				ptr += 2;
			    i = 0;
	    		memcpy(ptr, &i, 2);	// space for length
	    		ptr += 2;
				memcpy(ptr, &req->mID, 16);// UID Image
				ptr += 16;
				memcpy(ptr, &req->mDesiredDiscard, 1);// S8  DiscardLevel
				ptr += 1;
				i = 0;
				memcpy(ptr, &req->mImagePriority, 4);// F32 DownloadPriority
				ptr += 4;
				S32 packet = req->mLastPacket + 1;
				memcpy(ptr, &packet, 4);// U32 Packet
				ptr += 4;
				memcpy(ptr, &req->mType, 1);// U8  Type
				ptr += 1;
	
				length = ptr - smessg;
				send_packet(length, smessg, 21);	
...
LL_INFOS("AppInit") << "Reqst Img A" << LL_ENDL;
					...
					gMessageSystem->addU8Fast(_PREHASH_Type, req->mType);
 					llinfos << "OLD! IMAGE REQUEST: " << req->mID << " Discard: " << req->mDesiredDiscard
			 ...
			 iter1 != mCancelQueue.end(); ++iter1)
		{
			if (gSCTP == 0)
			{
				...
				for (queue_t::iterator iter2 = iter1->second.begin();
					 iter2 != iter1->second.end(); ++iter2)
				{
LL_INFOS("AppInit") << "Reqst Img B" << LL_ENDL;
					...
					gMessageSystem->addU8Fast(_PREHASH_Type, 0);
	 				llinfos << "OLD! CANCELING IMAGE REQUEST: " << (*iter2) << llendl;
					request_count++;
			...
			{
				// use sctp stream 5, type TPSC_IMGREQ
				unsigned char tmessg[50];
				unsigned char* ptr;
				int length;
				int i;
				...
				for (queue_t::iterator iter2 = iter1->second.begin();
					 iter2 != iter1->second.end(); ++iter2)
				{
LL_INFOS("AppInit") << "Cancl Img " << *iter2 << LL_ENDL;
					// assemble message
					ptr = tmessg;
					
					i = TPSC_IMGREQ;		// type login message
					memcpy(ptr, &i, 2);
					ptr += 2;
				    i = 0;
	    			memcpy(ptr, &i, 2);	// space for length
					...
					// *iter2 - pointer to uuid??
					memcpy(ptr, &(*iter2), 16);// UID Image ???
					ptr += 16;
					i = -1; 	// THIS IS A DISCARD
					memcpy(ptr, &i, 1);// S8  DiscardLevel
					ptr += 1;
					i = 0;
					memcpy(ptr, &i, 4);// F32 DownloadPriority
					ptr += 4;
					memcpy(ptr, &i, 4);// U32 Packet
					ptr += 4;
					memcpy(ptr, &i, 1);// U8  Type
					ptr += 1;
	
					length = ptr - tmessg;
//					send_packet(length, tmessg, 22);	Server cant handle
				} // end iter2 loop
			} // end messag etype
		} // end iter1
		mCancelQueue.clear();
...
{
//llwarns << "LLTextureFetchWorker::insertPacket  index:" << index << "  mTotalPackets:" << mTotalPackets << llendl;
	...
	if (index > 0 && index < mTotalPackets && size != MAX_IMG_PACKET_SIZE)
	{
...
{
llwarns << "LLTextureFetch::receiveImageHeader  " << id << "   packets=" << packets << llendl;

	LLMutexLock lock(&mQueueMutex);
	LLTextureFetchWorker* worker = getWorker(id);

llwarns << "LLTextureFetch::receiveImageHeader   last packet:" << worker->mLastPacket << llendl;
...
//llwarns << "LLTextureFetch::receiveImageHeader    codec:" << codec << llendl;
//llwarns << "LLTextureFetch::receiveImageHeader    packets:" << packets << llendl;
//llwarns << "LLTextureFetch::receiveImageHeader    totalbytes:" << totalbytes << llendl;

llwarns <<  "data_size: " << data_size << 
			"   FIRST_PACKET_SIZE:" <<  FIRST_PACKET_SIZE << 
			"   mFileSize: " << worker->mFileSize << llendl;
...
llwarns <<  "worker->insertPacket result = " << res << llendl;
...
if(res == 0){
	llwarns << "Inserted packet FAILED " << packet_num << "  result:" << res << llendl;
}
