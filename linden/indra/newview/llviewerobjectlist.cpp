...
void LLViewerObjectList::t_ProcessUpdateCore(LLViewerObject* objectp, 	//##kf from #672
											U64 region_handle,
											U16 time_dilation,
										    unsigned char *packet_bufferp, 
										    U32 obj_num)
{
	// ####
	objectp->t_ProcessUpdateMessage(region_handle, time_dilation, packet_bufferp, obj_num); // in llvoavatar.cpp | llviewerobject.cpp
...
{
	// ##kf Note that in llviewerobjectlist.h cached and compressed are defaulted to false. For incoming
	//     messages, update_type=OUT_FULL
	...
	if (!cached && !compressed && update_type != OUT_FULL)	//##kf This always fails?
	{
...
//		llinfos << "Received " << num_objects << " in " << size << " byte (" << size/num_objects << ")" << llendl;
	...
	for (i = 0; i < num_objects; i++)
	{
		...
		if (cached)	//##kf This always fails?
		{
			U32 id;
			U32 crc;
		...
		else if (compressed)	//##kf This always fails?
		{
			U8							compbuffer[2048];
		...
		else if (update_type != OUT_FULL)	//##kf This always fails?
		{
		...
		if (!objectp)	//##kf its a new object...
		{
			if (compressed)	//##kf This always fails?
			{
				if (update_type == OUT_TERSE_IMPROVED)
				{
			...
			else if (cached)	//##kf This always fails?
			{
			}
			else	//##kf This always runs?
			{
				if (update_type != OUT_FULL)	//##kf This always fails?
				{
// 					llinfos << "terse update for an unknown object:" << fullid << llendl;
					continue;
				}

				mesgsys->getU8Fast(_PREHASH_ObjectData, _PREHASH_PCode, pcode, i);	// ##kf What type of object is it?
			}
#ifdef IGNORE_DEAD
			...
			objectp = createObject(pcode, regionp, fullid, local_id, gMessageSystem->getSender());  //##kf <-------- make a new object!
			if (!objectp)
			{
				continue;
			}
...
void LLViewerObjectList::t_ProcessObjectUpdate(unsigned char *packet_bufferp)		//##kf from llviewermessage.cpp
{
	// ##kf processing of TPSS_AGTUPD,	// Agent terse update
	...
	// U64 RegionHandle	NOTE! Sent BIG-ENDIAN!
	memcpy(&region_handle, packet_bufferp, 8);
	packet_bufferp += 8;

	// U16 TimeDilation	
	memcpy(&time_dilation, packet_bufferp, 2);
	packet_bufferp += 2;
...
//U32 a = region_handle & 0xFFFFFFFF;
//U32 b = region_handle >> 32;
//llwarns << "TPSS_AGTUPD Reg = " << a << "," << b << "   TD=" << time_dilation << "    Flaga=" << region_flags << llendl;

	// U8 update block count
	num_objects = 0;
	memcpy(&num_objects, packet_bufferp, 1);
	packet_bufferp += 1;


//llinfos << "Received " << num_objects << " update in " << region_handle << " Region " << llendl;
	gFullObjectUpdates += num_objects;
	...
	for (i = 0; i < num_objects; i++)
	{
		...
		// UID full id
		memcpy(&fullid, packet_bufferp, 16);
		packet_bufferp += 16;

		// U32 Region LocalID
		memcpy(&local_id, packet_bufferp, 4);
		packet_bufferp += 4;

//llwarns << "   numObj=" << num_objects << "    fullid=" << fullid << "   LocID=" << local_id << llendl;

		objectp = findObject(fullid);

		if (!objectp)
		{
			llwarns << "Object update from unknown object! " << fullid << llendl;
			return;
		}
			
		// in Tritium 'local id' is unique to grid and does not need to change
		// on border crossing 
		...
		if (objectp->mLocalID != local_id)
		{
			llwarns << "Object update, local id mismatch! Update for " << local_id << "  but " 
				<< fullid << " is " << objectp->mLocalID << llendl;
			return;
		}
			
		if (objectp->getRegion() != regionp)
		{
			llwarns << "Object update, region mismatch! Update for " << regionp << "  but " 
				<< fullid << " is in  " << objectp->getRegion() << llendl;
			return;
		}
		...
		t_ProcessUpdateCore(objectp, region_handle, time_dilation, packet_bufferp, i);  //##kf 286
	}
...
// ##KF sctp handler routine for message TPSS_AVCREATE
LLViewerObject *LLViewerObjectList::processCreateAvatar(LLUUID fullid, U32 local_id, U64 region_handle, 
																U32 flags, LLHost sim)
{
	LLViewerObject *objectp;
	LLPCode			pcode = 0x2F;	// or maybe 0x20?
	LLViewerRegion *regionp;

LL_INFOS("AppInit") << " In  processCreateAvatar" << LL_ENDL;	
	objectp = findObject(fullid);
LL_INFOS("AppInit") << "    objectp=" << objectp << LL_ENDL;	
	if (objectp)
	{
		llwarns << "Attempt to create av that already exists! UUID=" << fullid  << llendl;
		return objectp;
	}
LL_INFOS("AppInit") << "    get regionp" 	 << LL_ENDL;
	regionp = LLWorld::getInstance()->getRegionFromHandle(region_handle);
LL_INFOS("AppInit") << "    regionp=" << regionp << LL_ENDL;	

	objectp = LLViewerObjectList::createObject(pcode, regionp, fullid, local_id, sim);

LL_INFOS("AppInit") << " objectp=" << objectp << " for " << fullid << LL_ENDL;	

	if (!objectp)
	{
		llwarns << "Create av failed! UUID=" << fullid  << llendl;
		return objectp;
	}
	objectp->mLocalID = local_id;
	
	// justCreated = TRUE;
	// the following ops are culled from these two routines:
	// processUpdateCore(objectp, user_data, i, update_type, NULL, justCreated);
	//     objectp->processUpdateMessage(msg, user_data, i, update_type, dpp);
	
	//mFlags &= FLAGS_LOCAL; cant ref from here?
	//mFlags |= flags;
	// info: State is a legacy field, unused in everything but grass. With 
	// grass it is used to determine species of grass	
	// = 0;
