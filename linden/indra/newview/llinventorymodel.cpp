...
//llwarns << "+++ addCategory " << category->getUUID() << llendl;		
...
void LLInventoryModel::t_LoadSkeleton(unsigned char *buffp, const LLUUID& owner_id)
{
	// handle SCTP message TPSS_INVSKEL and TPSS_LIBSKEL


	// TPSS_LIBSKEL,	// Library Skeleton					2
		// UID lib owner (removed in handle_stream7())
		// Repeating blocks of:
		// U32 type_default + version [=0 when no more blocks]
		// UID folder_id
		// UID parent_id
		// U16 length of name
		// Tv  name

	// TPSS_INVSKEL, 	// Inventory Skeleton entries		3
		// Repeating blocks of:
		// U32 type_default + version [=0 when no more blocks]
		// UID folder_id
		// UID parent_id
...
llwarns << "+++t_LoadSkeleton, owner=" << owner_id << llendl;	
	// U32 type_default + version [=0 when no more blocks]
	memcpy(&typev, buffp, 4);
	buffp += 4;
	...
	while(typev)
	{
		...
		// U16 length of name
		length = 0;
		memcpy(&length, buffp, 2);
		buffp += 2;
		
		// Tv  name 
		char fname[(length + 1)];
		char *fnamep = fname;
		strncpy(fnamep, (const char*)buffp, length);
		fname[length] = '\0';
		buffp += length;
llwarns << "++++ Imported folder '" << fname << "'  len=" << length << llendl;
		
		cat->rename(std::string(fnamep));
			
		type = typev & 0xFFFF;
		preferred_type = (LLAssetType::EType)type;
		...
		version = typev / 0x10000;
		cat->setVersion(version);
		temp_cats.insert(cat);
//llwarns << "     parent=" << cat->getParentUUID() << "   type=" << cat->getPreferredType() << "   Ver=" << version << llendl;
			
		// Get possible next typev
		memcpy(&typev, buffp, 4);
		buffp += 4;
	} // else there are no more folders in message
...
llwarns << "+++ Trying to open " << gzip_filename << llendl;
	...
	if (size > 30)
	{
llwarns << "+++ File exists... " << llendl;
		fclose(fp);
		fp = NULL;
		if (gunzip_file(gzip_filename, inventory_filename))
		{
llwarns << "+++ gunzipped to " << inventory_filename << llendl;
		...
		{
			llwarns << "Unable to gunzip " << gzip_filename << llendl;
		}
	}	// else no gzip file
...
llwarns << "+++ cached categories: " << count << llendl;
...
llwarns << "+++ cached_category_count: " << cached_category_count << llendl;
		for (cat_set_t::iterator it = temp_cats.begin(); it != temp_cats.end(); ++it)
		{
llwarns << "+++ Iterator 1 " << llendl; // this runs, does addCategory()
		...
		count = items.count();
//llwarns << "+++ cached items.count()=" << count << llendl;
		for (int i = 0; i < count; ++i)
		{
//llwarns << "+++ count 2 " << llendl; // this does not run as there is no cache
	...
	{
		// there was no zipped cache file so
		// add everything after stripping the version
		// information.
		for (cat_set_t::iterator it = temp_cats.begin(); it != temp_cats.end(); ++it)
		{
llwarns << "+++ Iterator 3 " << llendl;
...
//?llwarns << "+++ no_child_counts=" << child_counts.end() << llendl;

	update_map_t::iterator the_count;
	for (cat_set_t::iterator it = temp_cats.begin(); it != temp_cats.end(); ++it)
	{
//llwarns << "+++ Iterator 4 " << llendl;
	
		LLViewerInventoryCategory* cat = (*it);
//llwarns << "+++ Count Folder UUID " << cat->getUUID() << "   v=" << cat->getVersion() << llendl;				
		if (cat->getVersion() != NO_VERSION) // kf is this correct????
//		if(true)    This results in imported desc. count being set correctly, but still no inv displayed
		{
...
//llwarns << "    DescendentCount=" << cat->getDescendentCount() << llendl;				
		}
	}
	...
	if (remove_inventory_file)
	{
		...
		LL_DEBUGS("Inventory") << "Successfully loaded " << cached_category_count
			...
			<< LL_ENDL;
		llwarns << "+++ Summary: " << "Successfully loaded " << cached_category_count
			<< " categories and " << cached_item_count << " items from cache skeleton."
			<< llendl;
	}
	categories.clear(); // will unref and delete entries
			
//dumpInventory(); // kf test			
}

bool LLInventoryModel::loadSkeleton(const LLInventoryModel::options_t& options,		//#kf Old. Repl by t_LoadSkeleton above
...
llwarns << "++++ Imported folder '" << cat->getName() << "  type=" << preferred_type << "  ver=" << cat->getVersion() << llendl;
...
llwarns <<  "\nBegin Inventory Dump\n**********************:" << llendl;
	LL_DEBUGS("Inventory") << "\nBegin Inventory Dump\n**********************:" << LL_ENDL;
llwarns << "mCategroy[] contains " << mCategoryMap.size() << " items." << llendl;
	...
	for(cat_map_t::iterator cit = mCategoryMap.begin(); cit != mCategoryMap.end(); ++cit)
	{
		...
		if(cat)
		{
llwarns << "  " <<  cat->getUUID() << " '" << cat->getName() << "' "
...
llwarns << "mItemMap[] contains " << mItemMap.size() << " items." << llendl;
