...
extern int gSCTP;			// ##kft 0=not sctp, >0 using sctp
...
void LLHUDManager::t_processViewerEffect(unsigned char *buffp){

	llinfos << "Got viewer effect packet" << llendl;
	// Handle TPSS_EFFECTM, Multiple Effects
		// S32  Number of blocks
		// -- block start
		// UID uid of effect 
		// U8  Effect Type number
		// U8 item
	...
	LLUUID effect_id;
	U8 effect_type = 0;
	S32 number_blocks = 0; // extract froom buffp
	S32 k;
	U8 item = 0;	// not used?
	float duration;
	LLColor4U color;

	memcpy(&number_blocks, buffp, 4);			// S32  Number of blocks
	buffp += 4;	
	...
	for (k = 0; k < number_blocks; k++)
	{
		...
		memcpy(&effect_id, buffp, 16);			// UID uid of effect 
		buffp += 16;
		memcpy(&effect_type, buffp, 1);			// U8  Effect Type number
		buffp += 1;
		item = 0;
		memcpy(&item, buffp, 1);					// U8  item 
		buffp += 1;
		memcpy(&duration, buffp, 4);				// F32 Duration
		buffp += 4;
		memcpy(&color, buffp, 4);				// 4xU8 Color
		buffp += 4;

		S32 i;	// step thru current effects
		for (i = 0; i < LLHUDManager::getInstance()->mHUDEffects.count(); i++)
		{
			LLHUDEffect *cur_effectp = LLHUDManager::getInstance()->mHUDEffects[i];
			// clean out old effects?
			if (!cur_effectp)
			{
				llwarns << "Null effect in effect manager, skipping" << llendl;
				LLHUDManager::getInstance()->mHUDEffects.remove(i);
	...
	//			llwarns << "Dead effect in effect manager, removing" << llendl;
			...
			if (cur_effectp->getID() == effect_id)	// That id is in process
			{
				if (cur_effectp->getType() != effect_type)	// but its wrong type!
				{
					llwarns << "Viewer effect update doesn't match old type!" << llendl;
				}
				effectp = cur_effectp; // use the old pointer
				break;
			}
		} // done checking whats running currently

		if (effect_type)		// > 0
		{
			if (!effectp)	// effect_type was not already present
			{
			...
			if (effectp)
			{
				effectp->mID = effect_id;
				effectp->mType = effect_type;
				effectp->mDuration = duration;
				effectp->mColor	= color;
			}
		}
		else	// its a 0 !
		{
