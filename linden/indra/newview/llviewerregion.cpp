...
// ##kf for SCTP handling
void LLViewerRegion::setTerrainDetail(LLUUID terr_dtl_id_0, LLUUID terr_dtl_id_1, LLUUID terr_dtl_id_2, LLUUID terr_dtl_id_3)
{
	LLVLComposition *compp = getComposition();
	if (compp)	
	{
		compp->setDetailTextureID(0, terr_dtl_id_0);
		compp->setDetailTextureID(1, terr_dtl_id_1);
		compp->setDetailTextureID(2, terr_dtl_id_2);
		compp->setDetailTextureID(3, terr_dtl_id_3);
	}
}

// ##kf for SCTP handling
void LLViewerRegion::setTerrainStartHeight(F32 terr_strt_h_00, F32 terr_strt_h_01, F32 terr_strt_h_10, F32 terr_strt_h_11)
{
	LLVLComposition *compp = getComposition();
	if (compp)	
	{
		compp->setStartHeight(0, terr_strt_h_00);
		compp->setStartHeight(1, terr_strt_h_01);
		compp->setStartHeight(2, terr_strt_h_10);
		compp->setStartHeight(3, terr_strt_h_11);
	}
}

// ##kf for SCTP handling
void LLViewerRegion::setTerrainHeightRange(F32 terr_ra__00, F32 terr_ra__01, F32 terr_ra__10, F32 terr_ra__11)
{
	LLVLComposition *compp = getComposition();
	if (compp)	
	{
		compp->setHeightRange(0, terr_ra__00);
		compp->setHeightRange(1, terr_ra__01);
		compp->setHeightRange(2, terr_ra__10);
		compp->setHeightRange(3, terr_ra__11);
	}
}
// ##kf for SCTP handling
void LLViewerRegion::sendHandshakeReply(void)
{
	loadCache();
	
	// sctp msg type TPSC_REGHSREP
	unsigned char smessg[20];
	unsigned char* ptr;
	int length;
	int i;
	
	U32 x, y;	// get coords in meters
	from_region_handle(getHandle(), &x, &y);
	x /= 256;	// in region-count
	y /= 256;
...
LL_INFOS("AppInit") << "************** Sending region handshake reply for " << x << ":" << y << LL_ENDL;
	length = ptr - smessg;
	send_packet(length, smessg, 26);			
}
