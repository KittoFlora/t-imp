...
BOOL LLWearable::importFile( LLFILE* file )
{
	...
	S32 fields_read = 0;
//x llwarns << "/\\/\\/\\ importFile" << llendl; // ##kf
...
//x llwarns << "/\\/\\- mDescription =" << mDescription << llendl; // ##kf
...
//x llwarns << "/\\/\\- mPermissions =" << mPermissions << llendl; // ##kf
...
//x llwarns << "/\\/\\- mSaleInfo =" << mSaleInfo << llendl; // ##kf
	...
	if( fields_read != 1 )
	{
		llwarns << "Bad Wearable asset: no type!" << llendl;
		return FALSE;
	}
...
//x llwarns << "/\\/\\- mType =" << mType << llendl; // ##kf
...
//x llwarns << "/\\/\\- num_parameters =" << num_parameters << llendl; // ##kf
...
//x llwarns << "/\\/\\- num_textures =" << num_textures << llendl; // ##kf
...
llwarns << "========= Wearable asset: texture uuid: " << text_buffer << llendl;
...
void LLWearable::saveNewAsset()
{
	...
	if( gAssetStorage )
	{
		 ...
		 */
		 ...
		 data->mType = mType;
llwarns << "Call storeAssetData B llwearable.cpp" << llendl;
