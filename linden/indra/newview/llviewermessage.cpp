...
#include "llviewerimagelist.h"  //##kf added
...
extern int gSCTP;			// ##kft 0=not sctp, >0 using sctp, 2=connected, 3=sent name/pw, 4=logged in 
...
void kf_handle_layer(unsigned char *buffp){
	// stream 6 type TPSS_LAYRDATA
	// U16 region_x
	// U16 region_y
	// U8  layer type
	// U16 layerdata length
	// Tv  layerdata bytes
	...
	// Region the land data is for
	memcpy(&region_x, buffp, 2);
	buffp +=2;
	memcpy(&region_y, buffp, 2);
	buffp +=2;
...
//LL_WARNS("Messaging") << "TPSS_LAYRDATA kf_handle_layer sim= " << region_x << ":" << region_y << " " << sim << LL_ENDL;
	// find the regionpointer for it
	LLViewerRegion *regionp = LLWorld::getInstance()->getRegion(sim);

	if (!regionp || gNoRender)
	{
		LL_WARNS("Messaging") << "Region " << sim << " for Layer data does not exist." << LL_ENDL;
		return;
	}
	if (0 == size)
	{
...
//LL_WARNS("Messaging") << "Sim " << sim << "  Layer data type " << type << "   size " << size << LL_ENDL;

	U8 *datap = new U8[size];
	memcpy(datap, buffp, size);		// this skips the type and size info!
	LLVLData *vl_datap = new LLVLData(regionp, type, datap, size);
	gVLManager.addLayerData(vl_datap, size + 100);
//LL_WARNS("Messaging") << "Added Layer data type " << type << "   size " << size << LL_ENDL;

}

int kf_handle_image(int type, unsigned char *buffp){
	LLUUID id;
	U8 codec = 0;
	U16 packets = 0;
	U16 packet = 0;
...
//	LLHost host;
//	host.set("169.254.0.1", 5555);
//LL_INFOS("AppInit") << "Process Image header" << LL_ENDL;		
	
	if(type == TPSS_IMGHDR){					// image header
		memcpy(&id, buffp, 16);			// UID	Image uuid
		buffp += 16;
		memcpy(&codec, buffp, 1);			// U8	Codec
		buffp += 1;
		memcpy(&packets, buffp, 2);		// U16 Total blocks
		buffp += 2;
		memcpy(&totalbytes, buffp, 4);	// U32 total bytes
		buffp += 4;
		memcpy(&data_size, buffp, 2);		// U16 bytes following
		buffp += 2;
		memcpy(data, buffp, data_size);		// U8 image data
LL_INFOS("AppInit") << "Got Image header for " << id << " data size=" << data_size << LL_ENDL;		
		LLViewerImageList::SCTPreceiveImageHeader(id, codec, packets, totalbytes, data_size, data);
		repeat = 1;
		
	}else if(type == TPSS_IMGBLK){			// image body 
		memcpy(&id, buffp, 16);			// UID	Image uuid
		buffp += 16;
		memcpy(&packet, buffp, 2);		// U16 block number
		buffp += 2;
		memcpy(&data_size, buffp, 2);		// U16 bytes following
		buffp += 2;
		memcpy(data, buffp, data_size);		// U8 image data
//LL_INFOS("AppInit") << "Got Image block for " << id << LL_ENDL;
		LLViewerImageList::SCTPreceiveImagePacket(id, packet, data_size, data);
		repeat = 1;
		
	}else if(type == TPSS_IMGNA){		// image not available
		buffp += 2;						//skip type
		memcpy(&id, buffp, 16);			// UID	Image uuid
		LLViewerImageList::SCTPImageNotInDatabase(id);
		repeat = 0;
	}			
	return(repeat);
}

int kf_handle_wasset(unsigned char *buffp){
	// process a TPSS_WASSET
	LLUUID asset_id;
	U32 asset_type;
	S32 length;
	U8 data[MAX_MSG];
	...
	// UID	Asset id
	memcpy(&asset_id, buffp, 16);
	buffp += 16;
	// U8	Asset type
	asset_type = 0;
	memcpy(&asset_type, buffp, 1);
	buffp += 1;
	// S32  length
	memcpy(&length, buffp, 4);
	buffp += 4;
	// Tv	data
	memcpy(&data, buffp, length);
	buffp += length;
	dptr = data;
	// data contains whats in the wearable asset file
llwarns << "Received wearable asset  type=" << asset_type << "  id=" << asset_id << llendl;
	LLAgent::t_InitialWearableAssetArrived(asset_id, asset_type, dptr, length); // in llagent.cpp
...
void t_process_agent_movement_complete(F32 agent_pos_x, F32 agent_pos_y, F32 agent_pos_z, U8 look_at_x,  U8 look_at_y, U16 region_x, U16 region_y)
{
	...
	agent_pos[2] = agent_pos_z;
	look_at[0] = (F32)look_at_x;
	look_at[1] = (F32)look_at_y;
	look_at[2] = 0.0;
	...
	U32 ip = (((((n1 * 256) + n2) * 256) + 254) * 256) + 169;	// why does it have to be backwards?
	U32 pn = (U32) region_y;
	regionHost.set(ip, pn);
LL_INFOS("AppInit") << "Av in Region " << region_x << "." << region_y << " " << regionHost << LL_ENDL; //##kft
	
	gAgentMovementCompleted = true;

LL_INFOS("Messaging") << "t_process_agent_movement_complete()  IP=" << regionHost << LL_ENDL;
	...
	if (!avatarp)
	{
		...
		LL_WARNS("Messaging") << "t_process_agent_movement_complete() with NULL avatarp." << LL_ENDL;
	}

	F32 x, y;
	from_region_handle(region_handle, &x, &y);
	LLViewerRegion* regionp = LLWorld::getInstance()->getRegionFromHandle(region_handle);  // in world.cpp

LL_WARNS("Messaging") << " -------------- region_handle=" << region_handle << "  x:y=" << x << ":" << y 
			<< "  regionp=" << regionp << LL_ENDL;  

	if (!regionp)
	{
		if (gAgent.getRegion())
		{
			LL_WARNS("Messaging") << "Agent in region " << gAgent.getRegion()->getOriginGlobal() << LL_ENDL;
		}

		LL_WARNS("Messaging") << "Agent being sent to invalid region: " 
			<< x << ":" << y 
	...
	{
		LL_WARNS("Messaging") << "Agent starting in region: " << x << ":" << y 
			<< " current pos " << gAgent.getPositionGlobal()
			<< LL_ENDL;
	}
...
//	gViewerWindow->sendShapeToSim();  fov?
...
void process_agent_movement_complete(LLMessageSystem* msg, void**)
{
	...
	llwarns << "Sender " << msg->getSender() << LL_ENDL;  // ##kf
...
//		if (avatarp)
	...
	*/
LL_WARNS("process_agent_movement_complete") << "send_agent_update" << LL_ENDL;
	send_agent_update(TRUE, TRUE);
...
//	static U8 updc = 0;			//#kft avupdate rate limiter
//	static U8 MAXc = ;		//#kft avupdate rate limit
...
//			}
		...
		{			//#kft
			// sctp stream 1, type TPSC_AGNTUPD
			unsigned char smessg[MAX_MSG];
			unsigned char* ptr;
			int length;
			int i;
			float f32;
			...
			ptr += 2;
			f32 = body_rotation.mQ[VX];
			memcpy(ptr, &f32, 4);
			ptr += 4;
			f32 = body_rotation.mQ[VY];
			memcpy(ptr, &f32, 4);
			ptr += 4;
			f32 = body_rotation.mQ[VZ];
			memcpy(ptr, &f32, 4);
			ptr += 4;
			f32 = body_rotation.mQ[VS];
			memcpy(ptr, &f32, 4);
			ptr += 4;
			f32 = head_rotation.mQ[VX];
			memcpy(ptr, &f32, 4);
			ptr += 4;
			f32 = head_rotation.mQ[VY];
			memcpy(ptr, &f32, 4);
			ptr += 4;
			f32 = head_rotation.mQ[VZ];
			memcpy(ptr, &f32, 4);
			ptr += 4;
			f32 = head_rotation.mQ[VS];
			memcpy(ptr, &f32, 4);
			ptr += 4;
			memcpy(ptr, &render_state, 1);
			ptr += 1;
			memcpy(ptr, &flags, 1);
			ptr += 1;
			f32 = camera_pos_agent.mV[VX];
			memcpy(ptr, &f32, 4);
			ptr += 4;
			f32 = camera_pos_agent.mV[VY];
			memcpy(ptr, &f32, 4);
			ptr += 4;
			f32 = camera_pos_agent.mV[VZ];
			memcpy(ptr, &f32, 4);
			ptr += 4;
			v3 = LLViewerCamera::getInstance()->getAtAxis();
			f32 = v3.mV[VX];
			memcpy(ptr, &f32, 4);
			ptr += 4;
			f32 = v3.mV[VY];
			...
			memcpy(ptr, &f32, 4);
			ptr += 4;
			v3 = LLViewerCamera::getInstance()->getLeftAxis();
			f32 = v3.mV[VX];
			memcpy(ptr, &f32, 4);
			ptr += 4;
			f32 = v3.mV[VY];
			...
			memcpy(ptr, &f32, 4);
			ptr += 4;
			v3 = LLViewerCamera::getInstance()->getUpAxis();
			f32 = v3.mV[VX];
			memcpy(ptr, &f32, 4);
			ptr += 4;
			f32 = v3.mV[VY];
			...
			memcpy(ptr, &f32, 4);
			ptr += 4;
			memcpy(ptr, &gAgent.mDrawDistance, 4);
			ptr += 4;
			memcpy(ptr, &control_flags, 4);
			ptr += 4;
			
			length = ptr - smessg;
			send_packet(length, smessg, 25);
			
			gAgent.enableControlFlagReset(); 
			gAgent.resetControlFlags(); // ##KF added to clear controlFlagsDirty
		}
...
int t_process_avatar_create(unsigned char* sctp_buffp, LLHost sim){
...
//	LLViewerObjectList vol;
LL_INFOS("AppInit") << "Start decode TPSS_AVCREATE " << LL_ENDL; //##kft				
				
	memcpy(&fullid, sctp_buffp, 16);
	sctp_buffp += 16;
	memcpy(&local_id, sctp_buffp, 4);
	sctp_buffp += 4;
	memcpy(&region_x, sctp_buffp, 2);
	sctp_buffp += 2;
	memcpy(&region_y, sctp_buffp, 2);
	sctp_buffp += 2;
	memcpy(&flags, sctp_buffp, 4);
	sctp_buffp += 4;
	memcpy(&av_scale.mV[0], sctp_buffp, 4);
	sctp_buffp += 4;
	memcpy(&av_scale.mV[1], sctp_buffp, 4);
	sctp_buffp += 4;
	memcpy(&av_scale.mV[2], sctp_buffp, 4);
	sctp_buffp += 4;
			
LL_INFOS("AppInit") << "Decode TPSS_AVCREATE fullid=" << fullid << "  localID=" << local_id  << LL_ENDL; //##kft				
				region_handle = ((U64)region_x << 40) | ((U64)region_y << 8);
LL_INFOS("AppInit") << "Decode TPSS_AVCREATE Reg_hand=" << region_handle << "   flags=" << flags << "   scale=" << av_scale << LL_ENDL;


	// This puts the av in list gObjectList ??
	objectp = gObjectList.processCreateAvatar(fullid, local_id, region_handle, flags, sim); // in llviewerobjectlist.cpp

LL_INFOS("AppInit") << "---- processCreateAvatar returned pointer=" << objectp << LL_ENDL;

// test:
		LLViewerObject *testp1 = gObjectList.findObject(fullid);  // this returns 0!
		LL_INFOS("AppInit") << "---- findObject fullid returned pointer=" << testp1  << " for " << fullid << LL_ENDL;
//########## this pointer needs to be added to gObjectList 
...
void t_Process_object_update(unsigned char *packet_bufferp)		//##kf from llappviewer.cpp
	{
		gObjectList.t_ProcessObjectUpdate(packet_bufferp);		// in llviewerobjectlist.cpp
	}
...
void sctp_avatar_animation(unsigned char *buffp)
{
	LLUUID	animation_id;
	LLUUID	uuid;				// of target av
	S32 num_blocks;
...
LL_WARNS("Messaging") << ".................. Av Animation: " << LL_ENDL;
	// UID target avatar
	memcpy(&uuid, buffp, 16);
	buffp += 16;	
	avatarp = (LLVOAvatar *)gObjectList.findObject(uuid);
	if (!avatarp)
	{
		// no known agent with this ID...error?
		LL_WARNS("Messaging") << "Received animation state for unknown avatar: " 
				<< uuid << "   pointer=" << avatarp << LL_ENDL;
		return;
	}
	//clear animation flags
	avatarp->mSignaledAnimations.clear();
	
	// U8 animations block count
	num_blocks = 0;
	memcpy(&num_blocks, buffp, 1);
	buffp ++;
	/* not yet implemented
	// U8 source block count
	num_source_blocks = 0;
	memcpy(&num_source_blocks, buffp, 1);
	buffp ++;	
	// U8 events block count	
	num_events = 0;
	memcpy(&num_events, buffp, 1);
	buffp ++;	
	*/
	...
	if (avatarp->isSelf())
	{
		...
		{
			//   UID animation
			memcpy(&animation_id, buffp, 16);
			buffp += 16;	
			//   S32 sequence number
			memcpy(&anim_sequence_id, buffp, 4);
			buffp += 4;	

			LL_WARNS("Messaging") << "Animation id " << animation_id 
								   << " from self using sequence id " << anim_sequence_id << LL_ENDL;

			avatarp->mSignaledAnimations[animation_id] = anim_sequence_id;

			// this appears to be about an animation located inside an object, i.e. poseball?
			/* not yet implemented
	...
	{		// a nearby avatar
		for( S32 i = 0; i < num_blocks; i++ )
		{
			...
			buffp += 4;	
			avatarp->mSignaledAnimations[animation_id] = anim_sequence_id;
	LL_WARNS("Messaging") << "Received animation id " << animation_id 
								   << " from " << uuid 
...
{
	LL_ERRS ("Messaging") << "invalid_message_callback" << LL_ENDL;  //##kf  report some error
//    LLAppViewer::instance()->badNetworkHandler(); 		   //##kf this causes a crash in SCTP version
}
