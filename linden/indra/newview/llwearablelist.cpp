...
// static
LLWearable* LLWearableList::t_ProcessGetAssetReply(const LLAssetID& uuid, unsigned char* dptr, S32 length) 
//##kf tritium version of processGetAssetReply(), called from llagent.cpp
{
	LLWearable* wearable = NULL; // NULL indicates failure
	// convert message data to file
	std::string filename;
	filename = "/tmp/wasset_tmp";

	LLFILE* fp = LLFile::fopen(filename, "wb");
	size_t nwrit = fwrite(dptr, sizeof(U8), length, fp);
	fclose(fp); 
	if (nwrit < (size_t) length)
	{
		LL_WARNS("Wearable") << "Failed to write all of file for " << uuid << LL_ENDL;
	}
	else
	{
		// open file for read;
		fp = LLFile::fopen(filename, "rb");	
		wearable = new LLWearable(uuid);					// creates a new item 'wearable'
		bool res = wearable->importFile( fp );		 		// in llwearable.cpp, stuffs values from file in 'wearable'
		if (!res)
		{
			LL_WARNS("Wearable") << "Import of asset file for " << uuid << " failed." << LL_ENDL;
			delete wearable;
			wearable = NULL;
		}
		fclose( fp );
	...
	{
		LL_WARNS("Wearable") << "NO wearable loaded for asset " << uuid << LL_ENDL;
	}
	return wearable;
}
