...
void kf_handle_layer(unsigned char *buffp);				// ##kf
int kf_handle_image(int type, unsigned char *buffp); 	// ##kf
int kf_handle_wasset(unsigned char *buffp); 	// ##kf
...
void t_Process_object_update(unsigned char *packet_bufferp);
int t_process_avatar_create(unsigned char* sctp_buffp, LLHost sim);  //##kf for sctp
...
void sctp_avatar_animation(unsigned char *buffp);		//##kf
