...
extern int gSCTP;					//##kf 0=not sctp, >0 using sctp
int str0_type = 0;					//##kf If > 0  if there is a stream 0 message waiting
unsigned char packet_buffer[MAX_MSG];	//##kf message packet buffer
unsigned char *packet_bufferp;			//##kf pointer
int packet_type;						//##kf stream and type of incoming message
...
void request_initial_instant_messages()
{
	...
	{
		if (!requested && gAgent.getAvatarObject())
		{
			// sctp msg type TPSC_REQMSGS
			unsigned char smessg[50];
			unsigned char* ptr;
			int length;
			int i;
	
LL_INFOS("AppInit") << "Request Messages" << LL_ENDL;
			// assemble message
			ptr = smessg;
...
//##kf SCTP incoming message handlers... 
// returns 'repeat'
int handle_stream1(void){
	if (packet_type == TPSS_AGTUPD){
LL_INFOS("AppInit") << "++++ Handle TPSS_AGTUPD" << LL_ENDL;
	t_Process_object_update(packet_bufferp);		// in llviewermessage.cpp

	}else if (packet_type == TPSS_AGNTWUPD){
LL_INFOS("AppInit") << "++++ Handle TPSS_AGNTWUPD" << LL_ENDL;	
		LLAgent::t_AgentInitialWearablesUpdate(packet_bufferp);

	}else if (packet_type == TPSS_AGDATUD){
LL_INFOS("AppInit") << "++++ Handle TPSS_AGDATUD" << LL_ENDL;	
		LLAgent::t_ProcessAgentDataUpdate(packet_bufferp);

	}else if (packet_type == TPSS_AVANIM){
LL_INFOS("AppInit") << "++++ Handle TPSS_AVANIM" << LL_ENDL;	
		sctp_avatar_animation(packet_bufferp);
	}else if (packet_type == TPSS_EFFECTM){
//LL_INFOS("AppInit") << "++++ Handle TPSS_EFFECTM" << LL_ENDL;	
		LLHUDManager::t_processViewerEffect(packet_bufferp);
	}
	return(0);
}
int handle_stream2(void){
	return(0);
}
int handle_stream3(void){
	return(0);
}
int handle_stream4(void){
	return(0);
}
int handle_stream5(void){
	if( (packet_type == TPSS_IMGHDR) ||
		(packet_type == TPSS_IMGBLK) ||
		(packet_type == TPSS_IMGNA) ){
		return(kf_handle_image(packet_type, packet_bufferp));
	}else if (packet_type == TPSS_WASSET){
LL_INFOS("AppInit") << "++++ Handle TPSS_WASSET" << LL_ENDL;
		return(kf_handle_wasset(packet_bufferp));
	}else{
		return(0);
	}

}
int handle_stream6(void){
	if (packet_type == TPSS_LAYRDATA){
		kf_handle_layer(packet_bufferp);
		return(1);
	}else{
		return(0);
	}
}
int handle_stream7(void){
	if (packet_type == TPSS_LIBSKEL){
		memcpy(&gInventoryLibraryOwner, packet_bufferp,  16);
		packet_bufferp += 16;
LL_INFOS("AppInit") << "++++ Handle TPSS_LIBSKEL, owner is " << gInventoryLibraryOwner << LL_ENDL;	
		gInventory.t_LoadSkeleton(packet_bufferp, gAgent.getID());	 // in llinventorymodel.cpp
		return(1);
	}else if (packet_type == TPSS_INVSKEL){
LL_INFOS("AppInit") << "++++ Handle TPSS_INVSKEL" << LL_ENDL;	
		gInventory.t_LoadSkeleton(packet_bufferp, gAgent.getID());
		return(1);
	}else if (packet_type == TPSS_INVITEM){
		// kf_handle_asset(packet_bufferp);
		return(1);
	}else{
		return(0);
	}
...
void LLAppViewer::idle()
{
	...
	if ((gSCTP > 0) && (str0_type == 0) )	//##kf If using SCTP, check for messages
	{
		// we are using sctp, and there's no stream 0 packet waiting
		// so try to get a packet
		int repeat;
		
		do{ // If a packet is one of a block (eg. image or terrain transfer) then repeat 
			// is set to 1 and we loop here to fetch all packets quickly.
			repeat = 0;
			packet_bufferp = packet_buffer;
			packet_type = get_server_packet(packet_bufferp);
			int stream = packet_type & STREAMMASK;
			if (packet_type < 0)
			{	
				// server diconnected?
			}
			else if (packet_type > 0)
			{
				// we have a message
//int s = stream / 0x1000;
//int t = packet_type & 0xFFF;
if( (packet_type != TPSS_EFFECTM) &&
	(packet_type != TPSS_IMGBLK)  ){
  LL_WARNS("LLAppViewer") << ">-----> Incoming message, " << PacketName(packet_type) << llendl;
}
				packet_bufferp += 2; // skip type
				switch(stream)
				{
				case STREAM0:
					str0_type = packet_type;	// handled in idle_startup(...)
					// Setting str0_type > 0 stops further packet fetching until startup services this message
					break;		
				case STREAM1:
					repeat = handle_stream1();
					break;		
				case STREAM2:
					repeat = handle_stream2();
					break;		
				case STREAM3:
					repeat = handle_stream3();
					break;		
				case STREAM4:
					repeat = handle_stream4();
					break;		
				case STREAM5:
					repeat = handle_stream5();
					break;		
				case STREAM6:
					repeat = handle_stream6();
					break;		
				case STREAM7:
					repeat = handle_stream7();
					break;		
				default:
					LL_INFOS("AppInit") << "Unknown SCTP message, stream+type = " << packet_type << LL_ENDL;
				}
...
//LL_WARNS("AppInit") << "Idle_startup returned true" << LL_ENDL;	
...
//LL_WARNS("AppInit") << "Step E" << LL_ENDL;

	    // Handle automatic walking towards points
	    gAgentPilot.updateTarget();
	    gAgent.autoPilot(&yaw);
//LL_WARNS("AppInit") << "Step F" << LL_ENDL;
...
//LL_INFOS("AppInit") << "Idle - check flags" << LL_ENDL;	
//LL_WARNS("AppInit") << "Step F" << LL_ENDL;
//LL_WARNS("AppInit") << "Step G" << LL_ENDL;
    
	    if (flags_changed || (agent_update_time > (1.0f / (F32) AGENT_UPDATES_PER_SECOND)))
	    {
//LL_WARNS("AppInit") << "agent_update_time" << LL_ENDL;
//LL_WARNS("AppInit") << "send_agent_update 1   dirty=" << gAgent.controlFlagsDirty() 
//					<< "  last=" << last_control_flags << "  now=" << gAgent.getControlFlags() << LL_ENDL;
...
//LL_WARNS("AppInit") << "Step H" << LL_ENDL;
	...
	//
	//
//LL_INFOS("AppInit") << "Idle - stats" << LL_ENDL;	
...
//LL_WARNS("AppInit") << "Step I" << LL_ENDL;
...
//LL_WARNS("AppInit") << "Step J" << LL_ENDL;
...
//LL_WARNS("AppInit") << "Step K" << LL_ENDL;
	///////////////////////////////////////
...
//LL_WARNS("AppInit") << "Step L" << LL_ENDL;
	//////////////////////////////////////
	//
	// Deletes objects...
...
//LL_WARNS("AppInit") << "Step M" << LL_ENDL;
...
//LL_WARNS("AppInit") << "Step N" << LL_ENDL;
