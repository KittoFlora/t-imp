...
U32 LLVOAvatar::t_ProcessUpdateMessage(U64 region_handle,
									   U16 time_dilation,
										unsigned char *packet_bufferp, 
										U32 obj_num)
{
	LLMemType mt(LLMemType::MTYPE_AVATAR); // ?? not used

	LLVector3 old_vel = getVelocity();		// ?? not used
	// Do base class updates...
	U32 retval = LLViewerObject::t_ProcessUpdateMessage(region_handle, time_dilation, packet_bufferp, obj_num); 
				// in llviewerobject.cpp
...
void LLVOAvatar::updateMeshTextures()
{
	...
	if (!target_host.isOk())
	{
		llwarns << "updateMeshTextures: invalid host " << target_host << " for object: " << getID() << llendl;
	}
