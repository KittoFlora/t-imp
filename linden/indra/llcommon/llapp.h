...
class LLLiveFile;
#if LL_LINUX
typedef struct siginfo siginfo_t_i;
#endif
...
public:
	...
	typedef std::map<pid_t, LLChildInfo> child_map; // Map key is a PID
													//##kf cant be protected as used in llapp.cpp
	static child_map sChildMap;  //##kf cant be protected as used in llapp.cpp
#endif
...
//	typedef std::map<pid_t, LLChildInfo> child_map; // Map key is a PID  //##kf moved to public
...
#if !LL_WINDOWS
	friend void default_unix_signal_handler(int signum, siginfo_t_i *info, void *);
#endif
